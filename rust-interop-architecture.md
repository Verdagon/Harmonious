...
**Distinction from the F1 finding (2026-06-20).** The `#[inline(never)]` on Phase-6 wrappers above is a SEPARATE discipline from the historical `#[inline(never)]` on Sky-item stubs (accessor methods, wrapper functions, trait-impl methods in `stub_gen`). The historical attribute on Sky-item stubs was removed during F1 because: (a) its stated rationales (share-generics gate bypass, MIR inliner leak protection) were obsolete (patch 5 / §F.1 respectively), (b) its v2-precompiled-bodies concern (B12) is gated by build.rs in v1, and (c) it blocked LLVM's cross-language inliner. The Phase-6 wrappers' `#[inline(never)]` is retained because it serves an unrelated purpose: making the wrapper's symbol stably nameable from Sky's bitcode regardless of whether other Rust code happens to have called the wrapped stdlib operation. When v2 precompiled-bodies work begins, both `#[inline(never)]` policies should be re-evaluated together.
