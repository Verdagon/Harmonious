...
> **HISTORICAL DOCUMENT.** This guide describes the original single-crate proof-of-concept.
> The project has since been split into a two-crate workspace with a `LangCallbacks` trait.
> The concepts and mechanisms (query overrides, layout, MIR stubs, drop glue, borrow check skip)
> are still accurate — the code has been reorganized, not rewritten.
> See `rustc-lang-facade/README.md` for the current API.
...
│   │   └── registry.rs          # Parsed Toylang definitions (structs, functions
│   │                            #   with params, bodies, generic types, etc.)
│   ├── mir_helpers.rs           # Utilities for constructing MIR
│   ├── abi_helpers.rs           # ABI coercion (e.g. small-struct returns)
│   ├── codegen_wrapper.rs       # CodegenBackend wrapper, injects .o into link step
│   ├── file_loader.rs           # Custom FileLoader, injects generated Rust stubs
│   ├── llvm_gen.rs              # Compiles Toylang function bodies to LLVM IR
│   └── stub_gen.rs              # Generates Rust stub source from the registry
├── tests/
│   ├── point.toylang            # Test input
│   ├── counter.toylang          # Struct construction test
│   ├── pair.toylang             # Generic struct returns test
│   ├── host.rs                  # Rust code that uses Toylang types
│   ├── counter_test.rs          # Tests for counter.toylang
│   ├── pair_test.rs             # Tests for pair.toylang
│   └── layout_test.rs           # Layout verification tests
```
...
**Note:** The code above is the starting point. The actual implementation has grown beyond
this skeleton. In particular, `config` now also installs a custom `FileLoader` (via
`config.file_loader = Some(...)`) that injects generated Rust stubs so rustc never needs
a real `.rs` source file, and registers a `CodegenBackend` wrapper that injects a
Toylang-compiled `.o` file into the link step. The `after_analysis` callback does much more
than dump types — it drives LLVM IR generation for all Toylang function bodies, compiles
that IR to a native object file, and stores the `.o` path so the codegen wrapper can pick
it up later. See `file_loader.rs`, `codegen_wrapper.rs`, and `llvm_gen.rs` for details.
...
**Note:** The actual `ToyFunction` struct is significantly richer than what is shown above.
It includes `params` (parameter names and types), `return_ty`, `body` (a parsed AST of the
function body), `external_symbol` (the `__toylang_impl_*` symbol name used for linking),
`rust_deps` (Rust generic instantiations the function needs, e.g. `Vec::new`,
`Vec::push`), and `type_params` (for generic Toylang functions). The minimal version here
is enough to get the scaffolding working; the full definition lives in
`src/toylang/registry.rs`.
