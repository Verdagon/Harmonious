...
- **Sub-stage 5c.3 largely complete.** +2 Vec<consumer-type> migrations (stub_gen fix) + 8 more Vec<consumer-type> once the fix landed + 7 error-assertion tests + 5 callback-trace tests. Commit `1ae7fd4`. **121 of 129 migrated** at that point.
- **Sub-stage 5c.4 mostly complete.** +5 layout probe tests via new `run_integration_project_check_build_stderr` harness + augmented `lang_layout_of` log to include `size=N align=M`. **127 of 129 migrated total.** Only 2 remaining: `test_extern_fn_call` (bool bug, dedicated codegen fix) and `test_point_drop` (Rust-side `drop_in_place` + runtime.o, needs toylang.toml link-args support or facade-unit-test promotion). Both are parked with inline documentation in `integration_projects.rs`.
- **Sub-stage 5c.4 residue outstanding.** After deciding what to do with the 2 parked tests: retire direct mode + FileLoader + full doc pass.

**Current state:** 338/338 green (67 unit + 127 integration_projects + 129 integration legacy + 15 standalone).
...
| Rust-specific layout probes | 6 | **5 RESOLVED, 1 PARKED** | 5 migrated via `run_integration_project_check_build_stderr`: augment `lang_layout_of` log with `size=N align=M`, assert via `expected_build_stderr.txt` substring. Generic wrapper probes (`tg_of_vec_layout`, `tg_of_toypoint_layout`) need a `make_wrapper(...) -> ToyWrapper<T>` indirection to force rustc to query the wrapper's layout (inline construction doesn't trigger `layout_of` for the whole type). The 6th layout probe (`test_point_drop`) stays parked — uses `std::ptr::drop_in_place` + runtime.o, no wrapper-mode equivalent yet. |
...
| Rust-side-only (`std::ptr::drop_in_place` + runtime.o) | 1 | **PARKED** | `test_point_drop` uses `-C link-arg=runtime.o` + `std::ptr::drop_in_place(&mut p as *mut Point)` from Rust. No wrapper-mode path today; needs `toylang.toml` link-args support (build.rs would need to pass `-C link-arg=<path>` through to cargo's user_bin build) and a way to exercise Rust-side `drop_in_place` (arbitrary Rust in user_bin/main.rs is not supported — the shim is template-generated). Defer until either direction is worth pursuing. |
