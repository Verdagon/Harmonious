...
**Follow-on gotchas (unrelated to B6, flagged here because they share history).**

1. *Parallel-build contention under shared target dir.* Multiple concurrent `cargo build` invocations against a shared `CARGO_TARGET_DIR` wedge each other — tests report "running over 60 seconds" while every cargo subprocess waits on the `.cargo-lock`. Not a cargo bug, not a correctness issue; just coordination cost. The integration-test harness now takes a `BUILD_LOCK` mutex around each `toylangc build` invocation to make the serialization explicit at the test layer (one build at a time, binaries still run parallel afterwards). Future shared-target-dir orchestration should expect this contention and plan for either harness-level serialization or per-project target dirs (the latter costs `test_helpers` caching, which we rely on for the suite runtime budget). See `toylangc/tests/integration_projects.rs` `BUILD_LOCK` for the inline framing.

2. *Callback-trace test assertions remain incremental-incompat.* The `CARGO_INCREMENTAL=0` stopgap in the harness is preserved even post-B6. Not because B6 is unresolved — it is; the registry-driven codegen makes the consumer `.o` correct regardless of incremental-cache behavior. The remaining issue is that the 5 callback-trace tests (e.g. `test_deep_chain_only_entry_point_monomorphized`) assert invariants of the form "bork should NOT be monomorphized by rustc" — a property that holds under cold compile but breaks under rustc's incremental cache, which eagerly monomorphizes more pub fns in the rlib compile. The assertions predate the two-crate migration's shift to wrapper-mode rlib compile, and they need reformulating (weaker invariants, or a non-log signal) before the stopgap can be retired without regression. Separate test-suite followup; does not reopen B6.

**References.** Original diagnosis in commit `91cad25`. B6 architectural fix lives in `callbacks_impl.rs::populate_toylang_instances_from_cgus`; `generate_and_compile` calls it once before `generate_with_tcx` runs. Stateless `monomorphize_type` (required to avoid a separate re-entrant deadlock the B6 fix exposed) is in the same commit.
