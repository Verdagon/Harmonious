...
### B6. Consumer `.o` emission side-effect / incremental-cache interaction — RESOLVED

**Status:** Resolved in the post-stage-5 registry-driven codegen refactor. The former side-effect path (where `state.toylang_instances` was populated as a byproduct of `notify_concrete_entry_point` firing during `symbol_name` queries) moved to an up-front walk of the partitioner's CGU list in `generate_and_compile`. `notify_concrete_entry_point_inner` is now pure (aside from the log push) — it returns the extern symbol and nothing else. The new `populate_toylang_instances_from_cgus` helper in `callbacks_impl.rs` does the full walk deterministically each compile, regardless of which per-item queries cache-hit. The `CARGO_INCREMENTAL=0` stopgap in the integration-test harness is retired.

**What (historical).** Toylang's consumer `.o` was produced as a side effect of facade callbacks firing during rustc's query execution — specifically `notify_concrete_entry_point` (driven by `symbol_name`) populated `state.toylang_instances`, which `generate_with_tcx` consumed to emit the `.o`. Under rustc's incremental cache with a shared `CARGO_TARGET_DIR` and unchanged per-project sources, the per-item query cache-hit; the side-effecting walk didn't fire; `state.toylang_instances` stayed empty; the emitted `.o` contained only items discovered via the direct CGU walk (accessors). The rlib got rebuilt bundling an incomplete `.o`; downstream link failed with `__toylang_impl_main undefined` or similar.

**Why it didn't bite production (historical).** The preconditions required unchanged toylang sources between builds sharing a `CARGO_TARGET_DIR`. Typical `toylangc build` user flow (users edit `main.toylang` between builds) invalidates the cache correctly. The failure mode was specific to test harnesses running many unchanged-source projects through a shared target dir.

**Fix path taken.** Option 1 from the deferred-fix analysis: drive consumer codegen from the registry directly rather than relying on `notify_concrete_entry_point`'s side effects. The walk inputs are the partitioner's CGU list (always deterministic per compile — `collect_and_partition_mono_items` is a crate-wide query whose result is the source of truth) plus the consumer's registry (pure, immutable). The walk walks each entry-point consumer instance found in CGUs, resolves it to a `ToylangInstance`, and recursively walks its internal callees. No query-firing side effect; cache-hit on per-item queries is now irrelevant. Option 2 (`build.rs`-based `.o` tracking) was the fallback; not pursued since option 1 removes the fragility at its root.

**Follow-on gotcha (orthogonal to B6, flagged here because it shares history).**

*Parallel-build contention under shared target dir.* Multiple concurrent `cargo build` invocations against a shared `CARGO_TARGET_DIR` wedge each other — tests report "running over 60 seconds" while every cargo subprocess waits on the `.cargo-lock`. Not a cargo bug, not a correctness issue; just coordination cost. The integration-test harness now takes a `BUILD_LOCK` mutex around each `toylangc build` invocation to make the serialization explicit at the test layer (one build at a time, binaries still run parallel afterwards). Future shared-target-dir orchestration should expect this contention and plan for either harness-level serialization or per-project target dirs (the latter costs `test_helpers` caching, which we rely on for the suite runtime budget). See `toylangc/tests/integration_projects.rs` `BUILD_LOCK` for the inline framing.

**Side-effect retirement of stateful `monomorphize_type`.** The B6 fix exposed a re-entrant deadlock path: `lang_layout_of → call_monomorphize_type → lock MUTABLE_STATE` from inside `generate_and_compile` (whose trampoline already holds MUTABLE_STATE). Under cold compile the deadlock didn't fire because `layout_of` ran during rustc's mono walk (outside the mutex) and cached before `generate_and_compile`; under incremental-on warm cache the query skips and fires inside `generate_and_compile`. Resolved by making `monomorphize_type` stateless — drop the `&mut dyn Any state` param, let the dispatcher skip the lock. The former `CallbackLog::MonomorphizeType` entry was retired (not consumed by any test). Same commit as B6.

**Callback-trace test assertion reformulation.** Five callback-trace tests (e.g. `test_deep_chain_only_entry_point_monomorphized`) originally asserted "bork should NOT be monomorphized by rustc" — a property that held under direct-mode + cold compile but doesn't hold under wrapper-mode rlib compile: rustc's mono collector walks every `pub fn` in the rlib regardless of reachability. Orthogonal to B6; the invariant was direct-mode-specific. The harness's `unexpected` parameter is now a no-op (the positive-entries check still verifies B6's populate step runs correctly). Future work may reintroduce a weaker "these fns don't appear as rust-called entry points" check under a different signal.

**References.** Original diagnosis in commit `91cad25`. B6 architectural fix lives in `callbacks_impl.rs::populate_toylang_instances_from_cgus`; `generate_and_compile` calls it once before `generate_with_tcx` runs. Stateless `monomorphize_type` (required to avoid a separate re-entrant deadlock the B6 fix exposed) is in the same commit. `CARGO_INCREMENTAL=0` stopgap retired.
