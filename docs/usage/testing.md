...
Practical commands for building erw and running tests. As of the stage-4 zero-fork landing, erw builds against vanilla `nightly-2026-01-20` installed via rustup.
...
cargo +nightly-2026-01-20 test -p toylangc --test integration_projects

# Just unit tests (embedded in the toylangc binary)
cargo +nightly-2026-01-20 test -p toylangc --bin toylangc
...
```bash
cargo +nightly-2026-01-20 check -p toylangc
```
...
cargo +nightly-2026-01-20 run --bin toylangc \
...
The `nightly-2026-01-20` pin is intentional. When bumping:
