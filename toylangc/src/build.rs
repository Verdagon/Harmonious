...
fn write_stub_crate(
    ...
    // Sidecar→cache migration (2026-06-28, Step 1.5): each stub crate
    // gets a generated `build.rs` that emits `cargo:rerun-if-*` lines
    // for every cache-key axis. This makes cargo invalidate the stub
    // rlib's compile when any cache input changes, which in turn
    // causes the producer-side cache write to re-emit the
    // `.sky-cache` file with a fresh digest.
    cargo.push_str("build = \"build.rs\"\n");
    cargo.push_str("\n[lib]\n");
    cargo.push_str(&format!("name = \"{}\"\n", crate_name));
    ...
    // Sidecar→cache migration (2026-06-28, Step 1.5): emit a generated
    // `build.rs` that prints `cargo:rerun-if-*` lines for every
    // cache-key axis. The lines are computed against absolute paths
    // (skyc-time-known) so the build.rs is a thin one-shot that
    // doesn't need to re-parse the toylang.toml at cargo-build time.
    //
    // Wrapper mode's RUSTC_WORKSPACE_WRAPPER mtime path already
    // handles skyc binary invalidation natively; this build.rs adds
    // the source/manifest/annotation axes that cargo's standard
    // fingerprint mechanism doesn't reach.
    //
    // Plan Decision 3 / CACHE_KEY_AXES single-source-of-truth: the
    // axes covered here match `cache_key::build_rs_rerun_lines`. The
    // meta-test `cache_key_axes_and_build_rs_lines_are_in_sync`
    // guards against drift at the unit level; this emission site is
    // exercised by integration builds.
    let abs_source_for_rerun =
        std::fs::canonicalize(&source_path).unwrap_or_else(|_| source_path.clone());
    let abs_manifest_for_rerun = std::fs::canonicalize(&proj.project_dir.join("toylang.toml"))
        .unwrap_or_else(|_| proj.project_dir.join("toylang.toml"));
    let mut build_rs = String::new();
    build_rs.push_str("// Auto-generated by skyc (toylangc build.rs::write_stub_crate).\n");
    build_rs.push_str("// Sidecar→cache migration (Plan Decision 3 / Step 1.5): print\n");
    build_rs.push_str("// `cargo:rerun-if-*` lines for every cache-key axis so cargo\n");
    build_rs.push_str("// invalidates this stub rlib's compile when any cache input\n");
    build_rs.push_str("// changes. Mirrored against `toylangc::cache_key::CacheKeyAxis`.\n");
    build_rs.push_str("// DO NOT EDIT BY HAND — skyc rewrites this on every build.\n");
    build_rs.push_str("fn main() {\n");
    // LocalSourceHashes axis: rerun on any .toylang source change.
    build_rs.push_str(&format!(
        "    println!(\"cargo:rerun-if-changed={}\");\n",
        abs_source_for_rerun.display(),
    ));
    // SkyTomlHash axis: rerun on toylang.toml change.
    build_rs.push_str(&format!(
        "    println!(\"cargo:rerun-if-changed={}\");\n",
        abs_manifest_for_rerun.display(),
    ));
    // SkycBinaryHash axis: the wrapper binary's mtime rides cargo's
    // RUSTC_WORKSPACE_WRAPPER fingerprint path natively; the env
    // var below is a belt-and-suspenders override for any
    // non-wrapper invocation path (e.g. Sky-proper post-Phase-G
    // cdylib).
    build_rs.push_str("    println!(\"cargo:rerun-if-env-changed=SKYC_BINARY_HASH\");\n");
    // FormatVersion axis: bumping cache::CACHE_FORMAT_VERSION
    // invalidates every cache entry; signal via env var.
    build_rs.push_str("    println!(\"cargo:rerun-if-env-changed=SKYC_CACHE_FORMAT_VERSION\");\n");
    // TargetTriple axis: cargo's per-target cache key already
    // partitions outputs by triple; the env vars provide an
    // additional explicit dependency so a target change inside a
    // single target dir (rare; mostly relevant for cross-compile
    // toggles) also invalidates.
    build_rs.push_str("    println!(\"cargo:rerun-if-env-changed=CARGO_CFG_TARGET_ARCH\");\n");
    build_rs.push_str("    println!(\"cargo:rerun-if-env-changed=CARGO_CFG_TARGET_OS\");\n");
    build_rs.push_str("    println!(\"cargo:rerun-if-env-changed=CARGO_CFG_TARGET_ENV\");\n");
    build_rs.push_str("    println!(\"cargo:rerun-if-env-changed=CARGO_CFG_TARGET_VENDOR\");\n");
    // AnnotationFileHashes axis: currently empty in toylang. If a
    // future stub_crate carries annotation files, append
    // `cargo:rerun-if-changed=<path>` lines here. The slot is left
    // as a comment so the meta-test can detect a real annotation
    // file getting added without a build.rs update.
    build_rs.push_str("    // AnnotationFileHashes: no entries in current toylang.\n");
    // UpstreamCacheDigests: handled by cargo's natural rlib
    // fingerprint mechanism — when an upstream rlib changes, cargo
    // rebuilds this stub rlib, and the rebuild reads each upstream's
    // .sky-cache digest fresh. No `rerun-if-changed` line needed.
    build_rs.push_str("    // UpstreamCacheDigests: covered by cargo's rlib fingerprint.\n");
    build_rs.push_str("}\n");
    fs::write(stubs_dir.join("build.rs"), build_rs)
        .map_err(|e| format!("cannot write {}/build.rs: {}", stubs_dir.display(), e))?;
