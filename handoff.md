...
- ~~Phase Q (codegen_fn_attrs override for NEVER_UNWIND)~~ — **SHIPPED 2026-06-25 (commit `736eeb5`)**. Override stamps `NEVER_UNWIND` on `#[toylang::emit_consumer_body]`-tagged items; cache_audit fence enrolled. Extensions (`Cold`, `FFI_PURE`/`FFI_CONST`, `target_features`) deferred until measured-via-bench. Panic=unwind perf bench remains pending (no current bench exercises landing-pad emission).
...
7. ~~Bench 4 artifactual~~ — **resolved 2026-06-25 (commit `ae014d0`).** Confirmed: disassembly inspection showed LLVM completely inlined and constant-folded the inner loop; the "0.1% delta" was loop-overhead noise. Bench 4b (commit `ae014d0`) adds `std::hint::black_box` between maker + reader to defeat folding. **Critical surprise during IR inspection:** the Bench 4 disassembly revealed Sky was emitting `str wzr` (32-bit zero stores) for `i64` field initializers in struct literals — a silent miscompile from a parser/type-resolver bug where unsuffixed integer literals defaulted to `i32` and weren't widened against the expected field type. Fixed in the same commit (`type_resolve::resolve_expr`'s `IntLit` arm now widens i32 → i64/usize against expected_ty; narrowing still errors). Regression fixture at `test_drop_intlit_widening_struct_field`. Post-IntLit-fix, Bench 4b shows Sky at ~10% slower than inlineable Rust (vs. 38% pre-fix). The path-b conclusion holds for small inlineable bodies; remaining 10% gap may need path-b emission or further bench shapes to fully characterize. **Closes verification gap #7 AND found a real silent-miscompile bug in the process.**
