...
/// Check if a DefId is from the consumer's `__lang_stubs` rlib.
///
/// Under the current two-crate architecture the stub items live in their
/// own rlib crate named `__lang_stubs`. Both the rlib's own compile
