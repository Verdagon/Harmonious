...
- **`rustc_codegen_llvm`'s `pub(crate)` boundary is characterized, not a wall.** I'd earlier described this as "the one real wall" in this approach. A focused spike (branch `spike/modulellvm-wall`) showed that while item-level interception *inside* LLVM's CGU codegen path is blocked by `pub(crate)` types (`CodegenCx`, `Builder`, `ModuleLlvm` constructors), a coarser design sidesteps the wall entirely: override `collect_and_partition_mono_items` via `Config::override_queries`, delegate to upstream's partitioner, then filter consumer DefIds from the returned CGUs. Rust CGUs go to `LlvmCodegenBackend::codegen_crate` unchanged; consumer items are codegenned separately; outputs combine at `join_codegen`. No upstream changes needed. The spike's `findings.md` has the full walk-through including cross-boundary references.
...
- **`-Zcodegen-backend=valec` + `rustc_codegen_llvm` delegation:** 2-3 weeks of architecture work. **The `pub(crate)` boundary in `rustc_codegen_llvm` is characterized, not a wall** — the spike (`spike/modulellvm-wall`) showed that while item-level interception inside LLVM's CGU codegen is blocked, a coarser partitioner-override design sidesteps it entirely using only sanctioned `Config::override_queries` hooks. Breakdown: ~150 LoC for the partitioner override (delegating to upstream's partitioner via `OnceLock<DefaultPartitionerFn>`, filtering consumer DefIds from returned CGUs), ~200 LoC for the plugin wrapper (`codegen_crate` + `join_codegen` overrides extending the existing `codegen_wrapper.rs` pattern), plus the `rustc_driver::Callbacks` → backend-lifecycle reconciliation. **Note: this line item also absorbs the separate-crate integration work** that would otherwise sit under patch-5 elimination — plugin mode subsumes the stub-rlib codegen-skip problem and the cross-crate consumer-DefId handling naturally, per POC #2 (§4.3 of the reasoning doc).

  *Optional parallel contribution, not on the critical path:* if you want to drive an upstream PR to unlock finer-grained (item-level) interception for future use cases, the spike drafts a ~30-line PR description. The highest-probability single change (empty-ifying the `LlvmCodegenBackend(())` tuple field) has ~80-85% fast landing; the harder changes (`ModuleLlvm::new`, `ModuleLlvm::llmod` `pub`) are ~25-35% weeks-level / ~60-70% within 6 months. None blocking — ship on the coarser design, treat the PR as a toehold that signals the direction is welcome.
...
2. **Build the `CodegenBackend` plugin integration** to resolve the emission problem. Use the **coarser partitioner-override design** from the `spike/modulellvm-wall` branch — override `collect_and_partition_mono_items`, delegate to upstream's partitioner via `OnceLock<DefaultPartitionerFn>` (mirroring the facade's existing `DEFAULT_*` pattern), filter consumer DefIds from the returned CGUs. Rust CGUs to `LlvmCodegenBackend::codegen_crate`; consumer items to the plugin's own backend path; combine at `join_codegen`. ~150 LoC for the partitioner, ~200 LoC for the plugin wrapper. No `pub(crate)` wall to work around, no upstream PR required. Reasoning doc §4.2 has the full treatment.
