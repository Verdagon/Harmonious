...
impl<'ctx> Module<'ctx> {
    ...
    /// Wrap a raw [`LLVMModuleRef`] as a `ManuallyDrop<Module<'ctx>>` whose
    /// `Drop` is suppressed. Use this when the underlying `LLVMModule` is
    /// owned by another component (e.g. rustc's `ModuleLlvm`) and should
    /// outlive the borrowed wrapper.
    ///
    /// # Safety
    ///
    /// - `module` must point to a valid `LLVMModule` in a live `LLVMContext`.
    /// - The caller must ensure the underlying `LLVMContext` (typically wrapped
    ///   via [`ContextRef::new`](crate::context::ContextRef::new)) outlives the
    ///   wrapper, and the wrapper does not outlive the chosen `'ctx` lifetime.
    /// - The returned wrapper must not be unwrapped via `ManuallyDrop::into_inner`
    ///   in a way that would let the real `Drop` run: that would call
    ///   `LLVMDisposeModule` and free a module the caller does not own.
    pub unsafe fn new_borrowed(module: LLVMModuleRef) -> std::mem::ManuallyDrop<Module<'ctx>> {
        std::mem::ManuallyDrop::new(unsafe { Module::new(module) })
    }
