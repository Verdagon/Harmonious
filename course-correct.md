...
| 17 | ✅ | `stub_gen.rs:59, 86–97, 117–123, 127, 148, 185` | `if !is_generic { … }` branches | Single universal path; zero type params is the degenerate case (CLAUDE.md "compiler law") — landed for the two purely-cosmetic branches (impl-block header, wrapper-fn header) via `generics_for_impl_block` + `fn_generics_clause` helpers, and (Session 11 Phase E completion) for the **struct shape** via the universal `pub struct Foo<P...>(PhantomData<(P...)>);` form. The struct-shape branch was previously gated by a rustc debuginfo ICE; Session 11 shipped the defensive clamp as fork patch 4 (`e67de69ef35` on `per-instance-mir`, struct + sibling union site) and PR draft at `phase-e-rustc-pr-draft.md`. The architecture fence now also scans stub_gen.rs. One divergence remains (extern decls, gated by `extern "C"` not permitting generics — Sky retires them entirely when #4's inline-codegen rewrite lands; properly marked `arch-fence-allow: extern-C-cannot-be-generic`). See `phase-e-investigation.md`. |
