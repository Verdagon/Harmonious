...
**Session 17 (separate from course-correct numbering)**: B10 risk (LLVM 21 bitcode-writer bug; commit `dc8a8e8`) mitigated in-process. Investigation pinned the bug below the Rust/C boundary — Inkwell's `Module::write_bitcode_to_*` and `Module::add_function` and `Builder::build_direct_call` are all thin FFI shims; the buggy code lives in LLVM 21's `BitcodeWriter.cpp`'s function-table dedup. Fix: emit IR text via `print_to_string()`, parse it back via `Context::create_module_from_ir` (LLVMParseIRInContext), and emit bitcode from the canonicalised re-parsed module — LLVM 21's own IR parser acts as the canonicalising filter. Stays in-process. `assemble_text_to_bitcode` shell-out helper + `find_sysroot_tool` deleted; vendoring not needed. Also sharpened case1b to use a Rust-user struct as T (matches case3's pattern), closing Session 9's documented soft-gap. 266/266 still passing.
...
| 3 | ✅ | `cgu_stash.rs` (whole file) | Walk rustc's unfiltered CGU slice for Sky-item discovery | **Done in commit `c0a83fe`** (Session 16). Accessors collapsed into the regular function pipeline via parse-time `synthesize_accessor_pairs`; Case-1b generic instantiations from Rust callers go through `default_collect_and_partition()(tcx, ())` called directly inside `codegen_crate` with live `'tcx`. `cgu_stash.rs` deleted; `stash_upstream_cgus` / `clear_upstream_cgus` / `upstream_cgus` retired. Architecture risks.md §B5 closed. |
...
| 8 | ✅ | `lib.rs:131–136` (`monomorphize_type`) | "Give me field types so rustc composes layout" | **Done in commit `63beb0c`** (Session 16). `SkyUniverse` extended to carry `struct_infos: HashMap<String, Arc<dyn Any + Send + Sync>>` for consumer-side struct metadata; layout queries cross-Sky-crate (Case 6) route through the universe instead of toylang's per-callbacks `upstream_structs` mutex mirror. The `monomorphize_type` callback retires the dual-source workaround; full Sky-side recursive walk is bundled with Sky's frontend work. |
...
| 12 | ✅ | `lib.rs:197–304, 415–465` | `MUTABLE_STATE` + two-vtable + `_inner` bypass | **Done as side effects + commit `b92f101`** (Session 16). `PredicateVtable` retired in #7.4; `notify_concrete_entry_point` + Session-5 thread-local fat-pointer bypass retired in #9; `generate_and_compile` retired in Phase 4.5 Path B via `extra_modules_hook` fork patch. No query provider can re-enter `MUTABLE_STATE` today. The mutex itself remains — load-bearing because `collect_generic_rust_deps` fires from `lang_per_instance_mir` on rayon worker threads — but its sole role is plain inter-callback serialisation, not @GCMLZ trap-fencing. Close-out commit refreshed @GCMLZ doc + lib.rs comments + inlined the single-field `FacadeMutableState` struct. |
